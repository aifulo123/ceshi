...
#获取业务下主机(没有集群条件)
def get_host_list_not_set(request):
    try:
        host_list = []
        biz_id = int(request.GET.get("biz_id"))
        ip = request.GET.get("ip")
        ...
        if ip:
            ip_list = ip.split(",")
        result = cc_search_host(biz_id,ip_list,"")
        if result["result"]:
            data_list = result["data"]["info"]
            for item in data_list:
                host = item["host"]
                ...
                hasip = HostData.objects.filter(ip=ip)
                if hasip.count()>0:
                    host["ismonitor"] = True
                host["biz"] = biz[0]["bk_biz_name"]
                host["biz_id"] = biz[0]["bk_biz_id"]
                host["yun"] = host["bk_cloud_id"][0]["bk_inst_name"]
                host["yun_id"] = host["bk_cloud_id"][0]["bk_inst_id"]
                host["memory"] = ""
                host["cpu"] = ""
                host["disk"] = ""
...
@login_exempt
@csrf_exempt
def set_host_monitor(request):
    try:
        params = json.loads(request.body)
        ip = params["ip"]
        hasip = host.objects.filter(ip=ip)
        if hasip.count()>0:
            hasip.delete()
        else:
            res = host()
            res.biz_id = int(params["biz_id"])
            res.host_name = params["host_name"]
            res.ip = ip
            res.is_monitor = True
            res.yun_id = int(params["cloud_id"])
            res.save()
        return render_json({"result":True,"data":u"执行完成"})
    except Exception, e:
        return render_json({"result":False,"data": u"系统异常，请联系系统管理员!","message":str(e)})

#获取主机曲线图数据
@login_exempt
def get_host_line_data(request):
    try:
        ip = request.GET.get("ip")
        now = datetime.datetime.now()
        start = ((now+datetime.timedelta(hours=-1)).strftime("%Y-%m-%d %H:%M:%S"))
        end = now.strftime("%Y-%m-%d %H:%M:%S")
        time_lists = HostData.objects.filter(ip=ip,when_created__lte=end,when_created__gte=start).values("when_created","cpu","memory","disk").order_by("when_created")
        time_list =[]
        cpu_list = []
        memory_list = []
        disk_list = []
        for item in time_lists:
            tem = item["when_created"].strftime("%Y-%m-%d %H:%M:%S")
            time_list.append(tem)
            cpu_list.append(item["cpu"])
            memory_list.append(item["memory"])
            disk_list.append(item["disk"])
        newData={
            "xAxis":time_list,
            "series":[
                {
        ...
        return render_json({"result":True,"data":newData})
    except Exception, e:
        return render_json({"result":False,"data": u"系统异常，请联系系统管理员!","message":str(e)})
